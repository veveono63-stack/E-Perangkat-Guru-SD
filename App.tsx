...
const App: React.FC = () => {
  ...
  const [isLoggingIn, setIsLoggingIn] = useState(false); // Flag to prevent dashboard flash on failed login

  useEffect(() => {
    const unsubscribe = onAuthUserChanged(async (firebaseUser) => {
      // Selama proses registrasi atau login, Firebase akan mengubah state auth sesaat.
      // Kita abaikan perubahan ini untuk mencegah kilatan UI yang tidak diinginkan dan memastikan notifikasi ditampilkan dengan benar.
  ...
  const handleLoginSuccess = (loggedInUser: User) => {
    ...
    setIsLoggingIn(false); // Reset flag on successful login
  };
  
  const handleRegistrationSuccess = (message: string) => {
    setSuccessMessage(message);
    setView('registerSuccess');
    setIsRegistering(false); // Nonaktifkan flag setelah proses selesai
  };
    ...
    if (user.role === 'admin') {
      ...
      return (
        <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4 text-center">
          ...
          <p className="mt-2 text-gray-600">Dasbor Guru sedang dalam pengembangan. Silakan kembali lagi nanti.</p>
          <button 
            onClick={handleLogout} 
            className="mt-6 inline-flex items-center px-4 py-2 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors"
          >
            Keluar
          </button>
        </div>
